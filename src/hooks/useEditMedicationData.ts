...
import type { MedicationWizardData } from '@/components/admin/wizard/types';
...
export const useEditMedicationData = (medicationId: string, isOpen: boolean) => {
  ...
  // Update local state when data is loaded - now handles missing administration data
  useEffect(() => {
    if (medication && indications !== undefined && contraindications !== undefined && dosing !== undefined) {
      console.log('Loading edit data for medication:', medication.medication_name);
      console.log('Administration data:', administration);
