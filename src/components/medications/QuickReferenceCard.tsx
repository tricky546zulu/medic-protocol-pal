      ...
      <CardHeader className="p-6 sm:p-8">
        <div className="flex flex-col sm:flex-row items-start justify-between gap-4">
          ...
          <Button 
            ...
            size="sm" 
            onClick={handlePrint}
            className="print:hidden flex-shrink-0 h-12 rounded-2xl bg-white/80 backdrop-blur-lg border border-violet-200/60 hover:border-violet-400/70 hover:bg-gradient-to-r hover:from-violet-50/90 hover:to-purple-100/80 transition-all duration-300 hover:scale-105 hover:shadow-lg shadow-violet-200/40 min-h-[44px] whitespace-nowrap"
          >
      ...
      <CardContent className="p-6 sm:p-8">
        <div className="space-y-6 sm:space-y-8">
          {dosing.map((dose) => {
            ...
            return (
              <div key={dose.id} className="bg-gradient-to-br from-gray-50/90 to-white/80 rounded-3xl p-5 sm:p-6 backdrop-blur-lg border border-gray-100/60 hover:shadow-lg transition-all duration-200 shadow-lg shadow-gray-200/50">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-5 gap-4">
                  <h4 className="text-base sm:text-lg font-bold break-words flex-1">{dose.patient_type}</h4>
                  <div className="flex flex-wrap gap-2 sm:gap-3 flex-shrink-0">
                    {dose.route && (
                      <Badge variant="outline" className="bg-white/80 border-gray-300/60 text-gray-800 text-xs sm:text-sm px-3 py-2 rounded-2xl backdrop-blur-lg hover:bg-white hover:scale-105 transition-all duration-200 font-semibold min-h-[32px] whitespace-nowrap">{dose.route}</Badge>
                    )}
                    {dose.requires_infusion_pump && (
                      <Badge variant="outline" className="flex items-center gap-2 text-blue-800 border-blue-300/60 bg-gradient-to-r from-blue-100/90 to-sky-200/80 text-xs sm:text-sm px-3 py-2 rounded-2xl backdrop-blur-lg hover:scale-105 transition-all duration-200 font-semibold min-h-[32px] whitespace-nowrap">
                ...
                {dose.concentration_supplied && (
                  <p className="text-xs sm:text-sm text-gray-700 break-words mb-4 bg-white/70 p-3 rounded-2xl font-medium">
                    Concentration: {dose.concentration_supplied}
                  </p>
                )}
                {dose.requires_infusion_pump && pumpSettings && (
                  <div className="mt-5 p-5 sm:p-6 bg-gradient-to-br from-blue-50/90 to-sky-100/80 rounded-3xl text-xs sm:text-sm backdrop-blur-lg border border-blue-100/60 hover:shadow-lg transition-all duration-200 shadow-lg shadow-blue-200/50">
